...
| NetCoreCI | FR-N.1 discover, N.2 check-and-repair, N.3 configurable steps, N.4 triggers, N.5 runner, N.6 build tool, N.7 tests+Sonar, N.11 validate+3-retry, N.12 PR (never merge) | N.8 SAST/SCA/DAST, N.9 Dynatrace/Splunk, N.10 Docker/SBOM/scan/Nexus/Helm |
| NetLegacyCI | FR-L.1/L.3/L.4/L.5 (Windows/MSBuild), tests+Sonar, validate+retry, PR | SAST/SCA/DAST, monitoring, MSI packaging |
| Cross-cutting | CR-2 (configurable steps), CR-3 (check-existing-repair), NFR-2 (no self-merge), NFR-3 (3-retry cap) | CR-1 critical-finding gates (need Fortify/Sonatype/Wiz), NFR-1 async scale-out |
...
- **Deterministic gate + bounded retry in code.** The 3-attempt cap and the
  check-existing-then-repair behaviour live in `ci-authoring/engine.py`, never
